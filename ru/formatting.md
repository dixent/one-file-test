...
Если ваш код содержит двойные фигурные скобки, что часто случается, если вы используете шаблоны [`amp-mustache`](../../../../documentation/components/reference/amp-mustache.md?format=websites) , вам необходимо обернуть часть кода:
...
<div class="ap-m-code-snippet">
  <pre>& lsqb; example preview = "default: none | inline | top-frame"<br>игровая площадка = "по умолчанию: истина | ложь"<br>импорт = "<custom-element-1> ,<custom-element-2> , ... "template ="<custom-template> "]` `html // пример кода` `& lsqb; / example]</custom-template></custom-element-2></custom-element-1></pre>
</div>
