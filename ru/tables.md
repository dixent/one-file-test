...
- Объявления
teaser:
  text: _Если вы хотите предложить изменения в стандарте, прокомментируйте [Intent
toc: правда
---
...
<tbody>
<tr>
<td>Должен использовать<code></code> или<code></code> в качестве закрывающих тегов.</td>
<td>Позволяет валидаторам идентифицировать творческий документ как общий документ AMP или как ограниченный рекламный документ AMPHTML и отправлять его соответствующим образом.</td>
</tr>
<tr>
...
<td>In some cases, AMPHTML ads may choose to render an ad creative in an iframe.In those cases, host page analytics can only target the entire iframe anyway, and won’t have access to any finer-grained selectors.<br><br> <p>Example:</p> <pre> <amp-analytics id="nestedAnalytics">   <script type="application/json">   {     "requests": {       "visibility": "https://example.com/nestedAmpAnalytics"     },     "triggers": {       "visibilitySpec": {       "selector": "amp-ad",       "visiblePercentageMin": 50,       "continuousTimeMin": 1000       }     }   }   </script> </amp-analytics> </pre> <p>This configuration sends a request to the <code>https://example.com/nestedAmpAnalytics</code> URL when 50% of the enclosing ad has been continuously visible on the screen for 1 second.</p> </td>
</tr>
</tbody>
</table>
...
<code>touch-action</code> запрещено.</td>
  ...
  <tr>
    ...
    <td>AMP должен иметь возможность управлять всеми анимациями, относящимися к объявлению, чтобы он мог останавливать их, когда реклама не отображается на экране или системные ресурсы очень низки.</td>
  </tr>
  <tr>
...
[исходный код: CSS] .box {преобразование: поворот (180 градусов); переход: преобразовать 2с; } [/исходный код]

**Плохо**

Свойство запрещено в классе CSS.

[исходный код: css] .box {цвет: красный; // свойство, не являющееся анимацией, запрещено в селекторе анимации transform: rotate (180deg); переход: преобразовать 2с; } [/исходный код]
...
[исходный код: css] переход: преобразование 2s; [/исходный код]

**Плохо**

[исходный код: css] переход: цвет фона 2s; [/исходный код]

**Хороший**

[исходный код: css] @keyframes Turn {from {transform: rotate (180deg); }

чтобы {преобразовать: повернуть (90 градусов); } } [/исходный код]

**Плохо**

[исходный код: css] @keyframes slidein {from {margin-left: 100%; ширина: 300%; }

до {маржа-слева: 0%; ширина: 100%; } } [/исходный код]
...
- amp-mraid, на экспериментальной основе. Если вы планируете использовать это, пожалуйста, сообщите о проблеме на [wg-ads](https://github.com/ampproject/wg-ads/issues/new) .
...
Большинство упущений связано либо с производительностью, либо с целью упрощения анализа объявлений AMPHTML.

*Пример:* `<amp-ad>` исключен из этого списка. Это явно запрещено, так как включение `<amp-ad>` внутри `<amp-ad>` может потенциально привести к неограниченным водопадам загрузки рекламы, что не соответствует целям эффективности объявлений AMPHTML.
...
*Пример:* `<amp-lightbox>` еще не добавлен, потому что даже некоторые объявления AMPHTML могут отображаться в iframe, и в настоящее время нет механизма для расширения объявления за пределы iframe. В будущем для этого может быть добавлена поддержка, если будет продемонстрировано желание.
...
- Как и в обычном документе AMP, тег объявления `<head>` должен содержать тег `<script async src="https://cdn.ampproject.org/amp4ads-v0.js"></script>` .
...
- В отличие от обычного AMP, `<script type="application/ld+json">` запрещен.
    - *Обоснование:* JSON LD используется для разметки структурированных данных на главных страницах, но рекламные объявления не являются отдельными документами и не содержат структурированных данных. Блоки JSON LD в них будут просто стоить пропускной способности сети.
